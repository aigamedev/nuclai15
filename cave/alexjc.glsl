...
// https://www.shadertoy.com/view/4lfXWH
...
vec2 map( in vec3 pos )
{
    ...
    // Long "stones" hanging from ceiling, high frequency wave noise
    // truncated by low frequency waves.    
	float stalactites = -0.5 +
        clamp(2.0 - sin(-pos.x*8.0) * cos(-pos.z*7.0) * 2.0,
              0.85 + 0.5 *(sin(-pos.x*2.4)+cos(+pos.z*2.5)),
              1.0);
    ...
    float ceiling_y =
        ...
        // periodic waves in the ceeling
        + 0.1 *(sin(+pos.x*2.3)+cos(-pos.z*2.7))
		// additive stones
	   	+ stalactites
	;
    ...
    // Small stones on the floor, high-frequency wave noise
    // also truncated by low frequency waves.
	float stones = -0.5 +
        clamp(1.0-0.25*sin(-pos.x*8.0) * cos(-pos.z*7.0),
              0.85 + 0.55 *(sin(-pos.x*2.4) + cos(+pos.z*2.5)),
              1.0);
    ...
    float floor_y =
        ...
        +1.0 - 0.25 * sqrt(pos.x*pos.x + pos.z*pos.z)
        // waves in the floor based on sinusoids
	 	+0.15 * sin(pos.x * 2.1) * cos((-pos.x+pos.z) * 2.2)
        // additive stones	
        + stones
    ;
...
{    
    // If the position is below -1.0 then we assume it's water and
    // use modified checkerboard colors accordingly.
	if (pos.y < -1.5)
    {
        float f = mod( floor(5.0*pos.z) + floor(5.0*pos.x), 2.0);
        return (0.4+0.25*f)*vec3(0.1, 0.2, 1.0);
    }
